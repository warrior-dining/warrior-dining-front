...
const host = "http://localhost:8080/api/user/reviews/reservation"
const ReviewComment = () => {
    ...
    useEffect(() => {
        if (reservationId) {
            axiosInstance.get(`/api/user/reviews/reservation/${reservationId}`) // 여기가 예약 정보를 가져오는 부분
                .then(res => {
    ...
    const handleSubmit = (e) => {
        ...
        axiosInstance.post(`/api/user/reviews/reservation/${reservationId}`, createData)
            .then(res => {
                // 성공 후 페이지 이동
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-group">
                            <label htmlFor="restaurant-name"
                                   id="restaurant-name-label"> {data.place && data.place.name ? data.place.name : "음식점 이름을 불러오는 중입니다..."}</label>
                        </div>
